...
function turnDraggedCellToObj() {
    if (mouseDown) {
        console.log(document.elementFromPoint(window.event.clientX, window.event.clientY));
...
function applyObj(x, y) {
    ...
    grid[y][x].div.className = "cell " + objClass;
    grid[y][x].type = currentObj;
}
function fill(cell, cellType) {
    var x = cell.x;
    var y = cell.y;
...
function createGrid() {
    ...
    for (var y = 0; y < h; y++) {
        for (var x = 0; x < w; x++) {
            ...
            cell.div.className = "cell neutral ";
            cell.div.addEventListener("mousedown", function () {
                mouseDown = true;
                turnCellToObj(this);
            }, false);
            cell.div.addEventListener("mousemove", turnDraggedCellToObj, false);
            cell.div.addEventListener("mouseup", function () {
                mouseDown = false;
            }, false);
            grid[y][x] = cell;
            canvas.appendChild(cell.div);
        }
    }
        ...
        for (var x = 0; x < w; x++) {
            if (grid[y][x].type == "road") {
                ...
                if (onEdge(x, y) || neighbourIs(x, y, "farm")) {
                    ...
                    locations.push(location_1, location_1, location_1, location_1);
                }
            }
        }
    }
...
function roadNetwork() {
    var network = create2DArray(w, h); // Road network is flipped vs grid for the A* implementation
    for (var y = 0; y < h; y++) {
        for (var x = 0; x < w; x++) {
            if (grid[y][x].type == "road") {
                network[x][y] = 1;
