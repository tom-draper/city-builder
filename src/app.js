...
function createArray(h, w) {
  return new Array(h).fill(null).map(function () {
...
function randomBlue() {
  return "rgb("
    .concat(randInt(28, 40), ",")
    .concat(randInt(165, 185), ",")
    .concat(randInt(240, 255), ")");
}
...
function tryCarDrive() {
  ...
  if (p >= Math.random()) {
    var _a = selectRandomLocation(startLocations),
      sx = _a[0],
      sy = _a[1];
    if (sx != null && sy != null) {
      var finishLocations = driveLocations();
      var _b = selectRandomLocation(
...
function nextFishingLocation(boat) {
  var _a = boatPosition(boat),
    sx = _a[0],
    sy = _a[1]; // Current boat location
  ...
  if (0.8 >= Math.random()) {
    ...
    var _b = selectRandomLocation(filterByDistance(sx, sy, finishLocations, 2)),
      fx = _b[0],
      fy = _b[1];
    if (fx != null && fy != null) {
      var finish = graph.grid[fx][fy];
...
function spawnAnimals() {
  if (0.05 >= Math.random()) {
    var _a = selectRandomLocation(farmLocations()),
      x = _a[0],
      y = _a[1];
    if (x != null && y != null) {
...
function moveUp(animal) {
  var _a = coordsToCell(animal.left, animal.top - animalBuffer),
    x = _a[0],
    y = _a[1];
...
function moveDown(animal) {
  var _a = coordsToCell(animal.left, animal.top + 5 + animalBuffer),
    x = _a[0],
    y = _a[1];
  if (grid[y][x].type == "grass") {
    animal.top = animal.top + animalMovement;
...
function moveLeft(animal) {
  var _a = coordsToCell(animal.left - animalBuffer, animal.top),
    x = _a[0],
    y = _a[1];
  if (grid[y][x].type == "grass") {
...
function moveRight(animal) {
  var _a = coordsToCell(animal.left + 5 + animalBuffer, animal.top),
    x = _a[0],
    y = _a[1];
  if (grid[y][x].type == "grass") {
    animal.left = animal.left + animalMovement;
...
function growForest() {
  if (0.1 >= Math.random()) {
    ...
    if (locations.length > 0) {
      ...
      var _a = locations[Math.floor(Math.random() * locations.length)],
        x = _a[0],
        y = _a[1];
